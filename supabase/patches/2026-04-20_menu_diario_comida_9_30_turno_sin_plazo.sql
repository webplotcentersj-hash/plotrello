-- Menú diario: pedido/cancelación del plato hasta 09:30 AR; cambio de turno de almuerzo sin límite horario.
...
CREATE OR REPLACE FUNCTION public.actualizar_solo_turno_menu(
  p_id_menu integer,
  p_id_usuario integer,
  p_turno_almuerzo smallint
)
...
BEGIN
  ...
  v_fecha_argentina := public.get_argentina_date();
  IF v_menu.fecha != v_fecha_argentina THEN
    RAISE EXCEPTION 'Solo se puede modificar el menú del día actual';
  END IF;
  ...
  WHERE id_menu = p_id_menu AND id_usuario = p_id_usuario;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'No tenés pedido registrado para el menú de hoy';
  END IF;

  IF v_result.turno_almuerzo = p_turno_almuerzo THEN
    RETURN v_result;
  END IF;
  ...
  SET turno_almuerzo = p_turno_almuerzo, fecha_seleccion = now()
  WHERE id_menu = p_id_menu AND id_usuario = p_id_usuario
  RETURNING * INTO v_result;

  RETURN v_result;
END;
$actualizar_solo_turno_menu$;

GRANT EXECUTE ON FUNCTION public.actualizar_solo_turno_menu(integer, integer, smallint) TO anon, authenticated;
