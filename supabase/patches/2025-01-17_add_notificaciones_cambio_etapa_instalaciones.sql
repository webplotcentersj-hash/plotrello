-- Trigger para notificar cuando cambia la etapa de Instalaciones
  ...
  IF OLD.etapa_instalaciones IS DISTINCT FROM NEW.etapa_instalaciones 
    ...
    IF operario_nombre IS NOT NULL THEN
      ...
      IF user_id_destino IS NOT NULL THEN
        notification_title := 'Cambio de etapa en Instalaciones';
    ...
    -- Notificar a todos los usuarios de instalaciones y administracion
    FOR usuarios_instalaciones IN 
      SELECT id, nombre 
      FROM public.usuarios 
      WHERE rol IN ('instalaciones', 'administracion', 'gerencia')
    ...
    LOOP
      BEGIN
        ...
        INSERT INTO public.user_notifications (
          ...
          usuarios_instalaciones.id, notification_title, notification_desc, 'info', NEW.id, false
        );
      EXCEPTION WHEN OTHERS THEN
        RAISE WARNING 'Error creando notificación para usuario %: %', usuarios_instalaciones.nombre, SQLERRM;
      END;
    END LOOP;
...
DROP TRIGGER IF EXISTS trigger_notify_cambio_etapa_instalaciones ON public.ordenes_trabajo;
CREATE TRIGGER trigger_notify_cambio_etapa_instalaciones
  AFTER UPDATE ON public.ordenes_trabajo
  FOR EACH ROW
  WHEN (OLD.etapa_instalaciones IS DISTINCT FROM NEW.etapa_instalaciones)
  EXECUTE FUNCTION public.notify_cambio_etapa_instalaciones();
