-- Señalética tótem: Presupuestos = llamado a presupuesto/asesor; Recepción = mostrador; Diseño gráfico y marketing = diseño.
...
BEGIN
  ...
  -- Presupuestos / Asesor = llamada a presupuesto (mismo título y roles que "pide asesor").
  v_es_solicitud_asesor :=
    v_key_lower = 'asesor'
    OR v_key_lower = 'presupuestos y asesoramiento'
    OR v_key_lower = 'presupuestos'
    OR coalesce(p_notas, '') LIKE '%[SOLICITUD_ASESOR_TOTEM]%';

  IF v_es_solicitud_asesor THEN
    v_titulo := 'Cliente en tótem pide asesor';
  ELSE
  ...
  v_room_id := CASE v_key
    ...
    WHEN 'Diseño gráfico y marketing' THEN 2
    WHEN 'Presupuestos y asesoramiento' THEN 5
    WHEN 'Recepción de pedidos' THEN 5
    WHEN 'Caja / Entrega de pedidos' THEN 5
    WHEN 'Marketing y comunicación' THEN 2
    WHEN 'Base de operaciones' THEN 6
    WHEN 'Entregas Taller Gráfico' THEN 6
    WHEN 'Entregas taller de imprenta' THEN 6
  ...
  v_rol_sector := CASE v_key
    ...
    WHEN 'Diseño gráfico y marketing' THEN 'diseno'
    WHEN 'Marketing y comunicación' THEN 'diseno'
    WHEN 'Presupuestos y asesoramiento' THEN NULL
    ...
    WHEN 'Entregas Taller Gráfico' THEN 'taller-grafico'
    WHEN 'Entregas taller de imprenta' THEN 'imprenta'
    WHEN 'Mostrador' THEN 'mostrador'
    WHEN 'Caja' THEN 'caja'
    WHEN 'Instalaciones' THEN 'instalaciones'
    WHEN 'Asesor' THEN NULL
    ELSE
      CASE v_key_lower
        WHEN 'mostrador' THEN 'mostrador'
  ...
  IF v_es_solicitud_asesor THEN
    -- Presupuestos: solo presupuesto/asesor/admin. Sector «Asesor»: también mostrador.
    FOR v_user IN
      SELECT id FROM public.usuarios
      WHERE rol IN ('asesor-tecnico', 'presupuestos', 'administracion')
         OR (v_key_lower = 'asesor' AND rol = 'mostrador')
    LOOP
    ...
    END LOOP;
  ELSIF v_rol_sector IS NOT NULL THEN
    FOR v_user IN
...
COMMENT ON FUNCTION public.crear_atencion_mostrador IS
  'Crea atención de mostrador; chat interno; notifica por sector. Presupuestos/Asesor = llamado a presupuesto; Recepción = mostrador; Diseño gráfico y marketing = diseño.';
