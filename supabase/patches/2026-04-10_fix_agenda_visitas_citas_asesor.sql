-- =============================================================================
-- AGENDA ASESOR TÉCNICO — SCRIPT COMPLETO (Supabase SQL Editor: pegar y ejecutar)
--
-- Compatible con el esquema actual (citas_asesor_tecnico ya existe con
-- telefono_contacto, recordatorio_telegram_15m_at; ordenes_trabajo.es_ficha_no_op).
-- No recrea la tabla: solo ALTER IF NOT EXISTS + RPCs + GRANT.
--
-- Nota: las políticas RLS antiguas que usan usuarios.roles (jsonb) no aplican si
-- la tabla usuarios solo tiene columna "rol" (text). La app usa RPC SECURITY
-- DEFINER; si necesitás SELECT directo desde el cliente, hay que alinear RLS.
-- =============================================================================
BEGIN;

-- Quitar todas las sobrecargas (p. ej. crear_cita_asesor con/sin telefono_contacto).
-- Sin esto: 42P13 al cambiar RETURNS, y 42725 en GRANT ("function name is not unique").
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- -----------------------------------------------------------------------------
-- 1. Columnas auxiliares (idempotente; en BD al día no hace nada)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 6. Permisos RPC (cliente con JWT = role authenticated)
-- -----------------------------------------------------------------------------
...
-- PostgREST (API REST de Supabase) cachea el catálogo. Sin esto suele aparecer:
-- "Could not find the function ... in the schema cache" aunque la función exista en public.
