-- Menú diario: pedido/cancelación del plato hasta 10:15 AR (antes 09:30).
-- Alinear con src/constants/menuDiario.ts (MENU_PEDIDO_HORA_TOPE_ARG).
...
BEGIN
  ...
  IF v_hora_actual > '10:15:00'::time THEN
    RAISE EXCEPTION 'El plazo para seleccionar el menú ha expirado. Debes hacerlo antes de las 10:15 AM (hora Argentina)';
  END IF;
...
BEGIN
  ...
  v_hora_actual := public.get_argentina_time();
  IF v_hora_actual > '10:15:00'::time THEN
    RAISE EXCEPTION 'El plazo para cancelar la selección ha expirado. Debes hacerlo antes de las 10:15 AM (hora Argentina)';
  END IF;
