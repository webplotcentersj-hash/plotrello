-- Crear tabla para historial de cambios de etapa en Taller de Imprenta
...
CREATE OR REPLACE FUNCTION public.registrar_cambio_etapa_taller_imprenta()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  IF OLD.etapa_taller_imprenta IS DISTINCT FROM NEW.etapa_taller_imprenta THEN
    ...
    IF OLD.etapa_taller_imprenta IS NOT NULL AND OLD.etapa_taller_imprenta != '' THEN
      ...
      WHERE id_orden = NEW.id
        AND etapa_nueva = OLD.etapa_taller_imprenta
      ...
      IF fecha_inicio_anterior IS NULL THEN
        fecha_inicio_anterior := COALESCE(
          OLD.etapa_taller_imprenta_fecha_inicio,
          now()
        );
      END IF;
    ...
    NEW.etapa_taller_imprenta_fecha_inicio := now();
...
DROP TRIGGER IF EXISTS trigger_registrar_cambio_etapa_taller_imprenta ON public.ordenes_trabajo;
CREATE TRIGGER trigger_registrar_cambio_etapa_taller_imprenta
  BEFORE UPDATE ON public.ordenes_trabajo
  FOR EACH ROW
  WHEN (OLD.etapa_taller_imprenta IS DISTINCT FROM NEW.etapa_taller_imprenta)
  EXECUTE FUNCTION public.registrar_cambio_etapa_taller_imprenta();
