-- ============================================
-- FIX: Corregir ambigüedad en función actualizar_presupuesto_cliente (v2)
-- ============================================

BEGIN;

-- Eliminar la función existente
DROP FUNCTION IF EXISTS public.actualizar_presupuesto_cliente(integer, jsonb, date, text, character varying);
...
BEGIN
  -- Verificar que el presupuesto existe y obtener su estado actual con referencia explícita
  SELECT EXISTS(SELECT 1 FROM public.presupuestos_clientes WHERE presupuestos_clientes.id = p_id_presupuesto),
         (SELECT presupuestos_clientes.estado FROM public.presupuestos_clientes WHERE presupuestos_clientes.id = p_id_presupuesto)
  ...
  SET
    fecha_vencimiento = COALESCE(p_fecha_vencimiento, public.presupuestos_clientes.fecha_vencimiento),
    observaciones_cliente = COALESCE(p_observaciones_cliente, public.presupuestos_clientes.observaciones_cliente),
    precio_total = precio_total_calculado,
    estado = COALESCE(p_estado, public.presupuestos_clientes.estado),
    fecha_envio = CASE 
      WHEN p_estado = 'enviado' AND estado_actual != 'enviado' THEN NOW()
      WHEN p_estado != 'enviado' THEN public.presupuestos_clientes.fecha_envio
      ELSE public.presupuestos_clientes.fecha_envio
    END,
    fecha_respuesta = CASE 
      WHEN p_estado IN ('aceptado', 'rechazado') AND estado_actual NOT IN ('aceptado', 'rechazado') THEN NOW()
      ELSE public.presupuestos_clientes.fecha_respuesta
    END,
    updated_at = NOW()
