...
-- Fix: `json_agg(...)` cannot be ordered directly without a subquery in this shape.
-- This function powers Mostrador/CRM ventas. It must return JSON array sorted by fecha_venta desc.
...
DECLARE
  v_result json;
BEGIN
  SELECT COALESCE(json_agg(t.row_obj), '[]'::json) INTO v_result
  FROM (
