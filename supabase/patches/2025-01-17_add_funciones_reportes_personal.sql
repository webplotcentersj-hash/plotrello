-- Funciones para generar reportes de personal
-- Para el sistema de Recursos Humanos

BEGIN;

-- Función para obtener estadísticas de actividad por usuario
...
BEGIN
  ...
  movimientos_usuario AS (
    ...
    WHERE hm.id_usuario = p_id_usuario
      AND (p_fecha_desde IS NULL OR hm.fecha_movimiento::date >= p_fecha_desde)
      AND (p_fecha_hasta IS NULL OR hm.fecha_movimiento::date <= p_fecha_hasta)
  ),
  ultima_actividad_usuario AS (
    SELECT MAX(hm.fecha_movimiento) as ultima_fecha
    FROM public.historial_movimientos hm
    WHERE hm.id_usuario = p_id_usuario
  ),
...
-- Función para obtener estadísticas por sector
...
BEGIN
  ...
  usuarios_periodo AS (
    ...
    FROM public.historial_movimientos hm
    WHERE hm.fecha_movimiento::date >= p_fecha_desde
      AND hm.fecha_movimiento::date <= p_fecha_hasta
  ),
...
COMMENT ON FUNCTION public.obtener_estadisticas_usuario IS
'Obtiene estadísticas de actividad y productividad de un usuario específico en un período determinado.';

COMMENT ON FUNCTION public.obtener_estadisticas_sector IS
'Obtiene estadísticas de actividad y productividad de un sector específico en un período determinado.';

COMMENT ON FUNCTION public.obtener_estadisticas_periodo IS
'Obtiene estadísticas generales de actividad y productividad en un período determinado.';
