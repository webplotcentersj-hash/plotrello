-- Evita duplicate key ux_ordenes_op_sector al convertir pedidos portal → OP.
-- El MAX+1 global no garantizaba que (numero_op, sector) estuviera libre.

CREATE OR REPLACE FUNCTION public.next_numero_op_disponible(p_sector text DEFAULT 'Diseño Gráfico')
RETURNS text
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
  SELECT COALESCE(
    MAX(
      CAST(
        NULLIF(substring(trim(ot.numero_op) FROM '^OP-([0-9]+)$'), '')
        AS bigint
      )
    ),
    0
  ...
  WHERE trim(ot.numero_op) ~ '^OP-[0-9]+$';
  ...
  LOOP
    ...
    IF v_intentos > 500 THEN
      RAISE EXCEPTION 'No se pudo asignar numero_op único para sector %', p_sector;
    END IF;

    v_candidato := v_candidato + 1;
    v_numero := 'OP-' || LPAD(v_candidato::text, 6, '0');

    EXIT WHEN NOT EXISTS (
      SELECT 1
      FROM public.ordenes_trabajo ot
      WHERE ot.numero_op = v_numero
        AND ot.sector IS NOT DISTINCT FROM p_sector
        AND ot.sector IS NOT NULL
        AND ot.sector <> 'Finalizado en Taller'
    );
  END LOOP;
...
COMMENT ON FUNCTION public.next_numero_op_disponible(text) IS
  'Siguiente OP-n disponible para un sector (respeta ux_ordenes_op_sector).';

GRANT EXECUTE ON FUNCTION public.next_numero_op_disponible(text) TO anon;
GRANT EXECUTE ON FUNCTION public.next_numero_op_disponible(text) TO authenticated;
...
DECLARE
  ...
  v_sector text;
BEGIN
  v_sector := COALESCE(NULLIF(trim(p_sector_inicial), ''), 'Diseño Gráfico');
  ...
  -- Idempotencia: pedido ya vinculado a OP (conversión previa incompleta en UI)
  IF pedido_record.id_op_asociada IS NOT NULL THEN
    SELECT ot.numero_op INTO numero_op_generado
    FROM public.ordenes_trabajo ot
    WHERE ot.id = pedido_record.id_op_asociada;
    ...
    IF FOUND THEN
      ...
      SET estado = 'convertido_completo'
      WHERE pc.id = p_id_pedido
        AND pc.estado IS DISTINCT FROM 'convertido_completo';

      RETURN QUERY
      SELECT
        pedido_record.id_op_asociada,
        numero_op_generado,
        'Pedido ya vinculado a OP ' || numero_op_generado,
        v_id_venta,
        v_numero_venta;
      RETURN;
    END IF;
  ...
  numero_op_generado := public.next_numero_op_disponible(v_sector);
...
COMMENT ON FUNCTION public.convertir_pedido_a_op IS
  'Convierte pedido web en OP; numero_op único por sector (next_numero_op_disponible).';
