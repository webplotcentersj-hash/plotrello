...
export default function ClienteCatalogoPage() {
  ...
  const [productosVisibles, setProductosVisibles] = useState(PRODUCTOS_POR_PAGINA)
  ...
  useEffect(() => {
    setProductosVisibles(PRODUCTOS_POR_PAGINA)
  ...
  const articulosMostrados = useMemo(
    () => articulosFiltrados.slice(0, productosVisibles),
    [articulosFiltrados, productosVisibles]
  )

  const hayMasProductos = articulosFiltrados.length > productosVisibles
  ...
  return (
    <ClientePageLayout className="cliente-catalogo-page">
      <ClientePageHeader
        ...
        actions={
          <button type="button" className="cliente-btn-primary" onClick={() => navigate('/cliente/carrito')}>
      ...
      {articulosFiltrados.length === 0 ? (
        ...
        <>
          <div className="resultados-info">
            <p>
              Mostrando {articulosMostrados.length} de {articulosFiltrados.length} producto
          ...
          {hayMasProductos && (
            <div className="catalogo-cargar-mas">
              <button
                type="button"
                className="cliente-btn-outline"
                onClick={() => setProductosVisibles((n) => n + PRODUCTOS_POR_PAGINA)}
              >
                Cargar más ({articulosFiltrados.length - productosVisibles} restantes)
              </button>
            </div>
          )}
