...
const MAX_CANDIDATES_IA = 80
const MAX_CV_BYTES = 4 * 1024 * 1024

export async function filterPostulacionesPlotAI(
  query: string,
  candidates: Array<{
    id: number
    nombre: string
  ...
  const list = candidates.slice(0, MAX_CANDIDATES_IA)
  const listText = list
    .map(
      (c) =>
...
export async function extractCvMetadataPlotAI(
  dataUrl: string,
  puestoPostulado?: string
): Promise<CvMetadataIa> {
  const parsed = stripDataUrl(dataUrl)
  if (!parsed) throw new Error('No se pudo leer el archivo del CV.')

  const bytes = (parsed.base64.length * 3) / 4
  if (bytes > MAX_CV_BYTES) {
    throw new Error('El CV supera 4 MB. Probá con un PDF más liviano.')
  }
  ...
  if (
    ...
    throw new Error('Formato no soportado para análisis IA. Usá PDF si podés.')
  }

  const puestoCtx = puestoPostulado ? `El candidato se postuló para: ${puestoPostulado}.` : ''

  const system = `Sos PlotAI, asistente de RRHH de Plot Center en Argentina. Analizá un CV.
...
SCHEMA:
{
  ...
  const text = await callGeminiGenerateContent({
    ...
    contents: [
      {
        role: 'user',
        parts: [{ text: system }, { inlineData: { mimeType: parsed.mimeType, data: parsed.base64 } }]
      }
    ]
  })
  ...
  const data = tryParseJsonFromModelText(text)
  if (!data) {
    throw new Error('PlotAI no devolvió un análisis válido. Reintentá con un PDF.')
  }
